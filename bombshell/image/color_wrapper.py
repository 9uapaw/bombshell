...
class ColorWrapper():
    ...
    def image_to_string(self, image: Image) -> []:
        ...
        for x in range(len(self.ADDON_DATA_SEQUENCE)):
            hexval = pixel_information[(x * self.colorpixel_size) + 5, self.xy_threshold + 10]
        ...
        converted_values.append(float(str(int(colors[1][1:7], 16)) + "." + str(int(colors[2][1:7], 16))))
        converted_values.append(float(str(int(colors[3][1:7], 16)) + "." + str(int(colors[4][1:7], 16))))
        ...
        converted_values.append(str(self.get_target_guid(colors[9][1:7] + colors[10][1:7] + colors[11][1:3])))
